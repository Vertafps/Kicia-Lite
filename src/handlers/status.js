...
async function maybeHandleStatusCommand(message, { refreshKb = forceRefreshKb } = {}) {
  ...
  if (nextStatus || fetchCommand || jarvisCommand || testProMaxCommand || statusCommand || policyCommand) {
    if (!canUseOwnerCommands(message)) return true;
  }
  ...
  if (statusCommand) {
    await safeReply(message, {
      embeds: [
        buildPanel({
          body: "usage: `$status`, `$status up`, `$status down`, `$status unaware`, `$fetch`, `$jarvis`, `$testpromax`, or `$policy [enable|disable|status]`",
          color: INFO
        })
      ],
      allowedMentions: { repliedUser: false }
